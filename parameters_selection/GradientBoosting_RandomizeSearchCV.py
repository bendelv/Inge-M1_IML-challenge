...
if __name__ == '__main__':
    ...
    param_dist = {
        ...
        "min_samples_leaf": [1, 0.0001, 0.0005, 0.001, 0.05, 0.1, 0.5 ],
    ...
    model1 = RandomizedSearchCV(model, param_distributions=param_dist, n_iter=n_iter_search, cv=5)
    scores = cross_val_score(model1, X_ls, y_ls, scoring= 'neg_mean_squared_error', cv=5, n_jobs = -1)
    ...
    with measure_time('Training'):
        print('Training...')
        model1.fit(X_ls, y_ls)


    print(model1.cv_results_ ['params'][model1.best_index_])
